...
1. The growth of the animal
2.Animal weight
3.The color of the animal's eyes
And methods:
1.Make a sound
2.Print information about the animal
Also implement the following interfaces that implement the Animal interface:
1.A pet with the following properties:
1.1 Nickname
1.2 Breed
1.3 Availability of vaccinations
1.4 Coat color
1.5 Date of birth
And methods:
1.5 Show affection
2. A wild animal with the following properties:
2.1 Habitat
2.2 Date of location
Implement classes of the following animals that implement interfaces of a pet or a wild animal (I hope you will understand for yourself which animal is domestic and which is wild):
1.A cat with the property:
1.1 The presence of wool (read-only to external classes)
2.1 Tiger
3. A dog with the property:
3.1 Availability of training (read-only to external classes)
and method:
3.2 Train
4.Wolf with the property:
4.1 Pack leader (read-only to external classes)
Also implement an abstract class "Bird" that implements the animal interface, which will have a protected field:
1.In
